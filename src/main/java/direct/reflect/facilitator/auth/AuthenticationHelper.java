...
/**
 * Authentication Helper for OIDC + Guest hybrid model.
 * 
 * Extracts user identity from HTTP sessions consistently for both:
 * - OIDC users (authenticated via external provider)  
 * - Guest users (anonymous with session-based identity)
 * 
 * This separates authentication concerns from business logic in ParticipantService.
 */
@Component
@Slf4j
