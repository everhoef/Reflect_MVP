...
public interface ParticipantResponseRepository extends JpaRepository<ParticipantResponse, UUID> {
    ...
    @Query("SELECT r FROM ParticipantResponse r"
        + " WHERE r.participant.session = :session AND r.retroStep = :step"
        + " ORDER BY r.displayOrder ASC, r.submittedAt ASC")
    List<ParticipantResponse> findBySessionAndRetroStep(
     ...
     */
    @Query("SELECT r FROM ParticipantResponse r"
        + " WHERE r.participant.session = :session AND r.retroStep = :step AND r.participant = :participant")
     ...
     */
    @Query("SELECT r FROM ParticipantResponse r"
        + " WHERE r.participant.session = :session AND r.retroStep = :step AND r.isVisible = true"
        + " ORDER BY r.displayOrder ASC, r.submittedAt ASC")
    List<ParticipantResponse> findVisibleBySessionAndRetroStep(
        @Param("session") RetroSession session,
        @Param("step") RetroStep step);
     ...
     */
    @Query(value = "SELECT * FROM participant_responses r"
        + " WHERE r.session_id = :#{#session.id} AND r.retro_step_id = :#{#step.id}"
        + " AND r.response_data->>'category' = :category"
        + " ORDER BY r.display_order ASC, r.submitted_at ASC",
        nativeQuery = true)
    List<ParticipantResponse> findCategoricalByCategory(
      ...
      */
     @Query("SELECT COUNT(r) FROM ParticipantResponse r"
        + " WHERE r.participant = :participant AND r.participant.session = :session AND r.retroStep = :step")
     Long countByParticipantSessionAndStep(
        @Param("participant") Participant participant,
        @Param("session") RetroSession session,
        @Param("step") RetroStep step);
    ...
    @Query("SELECT r FROM ParticipantResponse r"
        + " WHERE r.participant.session.id = :retroId AND r.clusterId = :clusterId")
    List<ParticipantResponse> findByRetroIdAndClusterId(
        @Param("retroId") UUID retroId,
    ...
    @Query("SELECT r FROM ParticipantResponse r"
        + " WHERE r.participant.session.id = :retroId AND r.clusterId IS NULL"
        + " ORDER BY r.displayOrder ASC, r.submittedAt ASC")
    List<ParticipantResponse> findByRetroIdAndClusterIdIsNull(@Param("retroId") UUID retroId);

    @Query("SELECT r FROM ParticipantResponse r"
        + " WHERE r.participant.session.id = :retroId AND r.clusterId IS NOT NULL"
        + " ORDER BY r.displayOrder ASC, r.submittedAt ASC")
