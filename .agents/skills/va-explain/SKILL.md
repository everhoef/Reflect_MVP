...
Once tests pass, handle all git operations silently. Follow this sequence exactly. Do not skip or reorder the safety checks.
...
**Files you expect to be dirty** are the CSV file(s) you just edited (one to three files under `src/main/resources/`).
...
| Generated TypeScript types | Auto-generated, not partner-owned |
...
Keep discarding until `git status --short` shows only the CSV file(s) you edited. Do not proceed past this point with anything else staged or present in the working tree.

#### 6b. Create a branch

Use a slugified, lowercase branch name based on what changed:
...
#### 6c. Stage only the exact CSV files you changed

Never use `git add .` or `git add -A`. Stage each file by its exact path:

```bash
git add src/main/resources/retrospective_steps.csv
```

Only add the specific file(s) you edited. If you changed two files:

```bash
git add src/main/resources/retrospective_steps.csv
git add src/main/resources/retrospective_stages.csv
```

#### 6d. Verify what is staged
...
This MUST show only the CSV file(s) you intended to commit. If anything else appears — any path at all — **do not commit**. Go back to 6a, restore the unexpected files, re-stage, and re-verify. Do not proceed until the cached diff shows only your intended files.
...
#### 6g. Return the PR link

Give the partner only the PR URL. Nothing else from this step needs to surface.
